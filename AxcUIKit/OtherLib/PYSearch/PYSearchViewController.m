...
- (void)tagDidCLick:(UITapGestureRecognizer *)gr
{
    ...
    self.searchSuggestionVC.searchKeyWord = label.text;
}
...
- (void)saveSearchCacheAndRefreshView
{
    ...
    if (self.showSearchHistory && searchText.length > 0 && searchText.length < 15) {
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText
{
    ...
    if ([self.delegate respondsToSelector:@selector(searchViewController:searchTextDidChange:searchText:)]) {
        ...
        self.searchSuggestionVC.searchKeyWord = searchText;
    }
}
