...
extension AxcBubbleView: AxcBubbleViewApi {
    ...
    public var bubbleContentEdgeInsets: UIEdgeInsets {
...
extension AxcBubbleView {
    func _set(contentEdgeInsets: UIEdgeInsets) {
    ...
    func _set(bubbleContentEdgeInsets: UIEdgeInsets) {
...
open class AxcBubbleView: AxcGradientView {
    ...
    func _updateMaskLayer() {
        ...
        // 箭头偏移限位边距
        let arrowOffsetEdgeLimit: UIEdgeInsets = _contentEdgeInsets.axc.add(size: _bubbleCornerRadius.axc.cgSize)
        // 根据方向留出空位
        var contentRectInsetEdge: UIEdgeInsets = .zero // 箭头的空位
        ...
        // 内容框
        let contentRect: CGRect = bounds.inset(by: _contentEdgeInsets)
        let roundedRect: CGRect = contentRect.inset(by: contentRectInsetEdge)
        let maskBezier: UIBezierPath = .init(roundedRect: roundedRect,
                                             byRoundingCorners: _bubbleCorners.toUIRectCorner,
                                             cornerRadii: _bubbleCornerRadius.axc.cgSize)
        // 从圆开始，顺时针
        let arrowRoundBezier: UIBezierPath = .init(arcCenter: arrowRoundCenter,
                                                   radius: _arrowRadius,
                                                   startAngle: startAngleRadian,
                                                   endAngle: endAngleRadian,
                                                   clockwise: clockwise) // 逆时针
        // 上方起始点
        arrowRoundBezier.addLine(to: startPoint)
        // 底部结束点
        arrowRoundBezier.addLine(to: endPoint)
        arrowRoundBezier.close() // 闭合图案
        // 两个贝塞尔合并
        maskBezier.append(arrowRoundBezier)
        _shapeLayer.path = maskBezier.cgPath // 赋值图案
    ...
    /// 气泡内容间距
    var _bubbleContentEdgeInsets: UIEdgeInsets = 4.axc.uiEdge
    /// 气泡圆角组
